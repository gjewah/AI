...
class FiqBefaringFunn(models.Model):
    ...
    befaring_id = fields.Many2one(
        "fiq.befaring", string="Befaring", required=True,
        ondelete="cascade", index=True)
    rom_id = fields.Many2one(
        "fiq.befaring.rom", string="Rom", ondelete="set null", index=True,
        help="Rommet funnet gjelder (valgfritt).")
    company_id = fields.Many2one(
        related="befaring_id.company_id", string="Firma",
        store=True, index=True, readonly=True)
    sequence = fields.Integer(string="Rekkefølge", default=10)
    name = fields.Char(string="Funn", required=True, help="Kort beskrivelse — navn, ikke ID.")
    beskrivelse = fields.Text(string="Beskrivelse")
    type = fields.Selection([
        ("observasjon", "Observasjon"),
        ("avvik", "Avvik"),
        ("endring", "Endring"),
    ], string="Type", default="observasjon", required=True, index=True)
    alvorlighet = fields.Selection([
        ("lav", "Lav"),
        ("middels", "Middels"),
        ("hoy", "Høy"),
    ], string="Alvorlighet", default="lav", index=True)
    status = fields.Selection([
        ("apen", "Åpen"),
        ("lukket", "Lukket"),
    ], string="Status", default="apen", required=True, index=True)
    foto = fields.Image(string="Foto", max_width=1920, max_height=1920)
