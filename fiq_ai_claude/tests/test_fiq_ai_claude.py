...
class TestClaudeProvider(TransactionCase):
    ...
    def test_leverandoer_er_registrert_i_odoos_providers(self):
        ...
        self.assertIn(
            "anthropic",
            navn,
            f"«anthropic» mangler i ai.utils.llm_providers.PROVIDERS: {navn}",
        )
    ...
    def test_registrering_er_idempotent(self):
        ...
        antall_for = sum(
            1 for p in _lp.PROVIDERS if getattr(p, "name", None) == "anthropic"
        )
        ...
        self.assertEqual(
            antall_for, antall_etter, "Leverandøren ble dublert ved ny registrering"
        )
        self.assertEqual(
            antall_etter, 1, "Det skal finnes nøyaktig ÉN anthropic-leverandør"
        )
    ...
    def test_odoo_finner_leverandoer_for_claude_modell(self):
        ...
        for modell, __ in ClaudeProvider.LLMS:
            self.assertEqual(
                _lp.get_provider(self.env, modell),
                "anthropic",
                f"Odoo fant ikke anthropic for modellen {modell}",
            )
    ...
    def test_anthropic_forurenser_ikke_embedding_utvalget(self):
        ...
        self.assertNotIn(
            "",
            modeller,
            f"Tom embedding-modell havnet i EMBEDDING_MODELS_SELECTION: {modeller}",
        )

    def test_respons_stil_mappes_til_modell(self):
        """Odoo velger modell via responsstil. Ukjent stil skal falle tilbake, ikke kaste."""
        self.assertEqual(
            ClaudeProvider.get_llm_model("slow_and_rigorous"), "claude-opus-4-8"
        )
        self.assertEqual(
            ClaudeProvider.get_llm_model("snappy_and_creative"), "claude-haiku-4-5"
        )
        self.assertEqual(ClaudeProvider.get_llm_model("standard"), "claude-sonnet-5")
        self.assertEqual(
            ClaudeProvider.get_llm_model("finnes-ikke"),
            "claude-sonnet-5",
    ...
    def test_ukjent_mimetype_degraderer_til_tekst_uten_aa_kaste(self):
        ...
        body = ClaudeProvider.build_message_body(
            ...
            ["u"],
            None,
            [{"mimetype": "application/x-ukjent", "value": 42}],
            None,
            [],
            1024,
        )
    ...
    def test_strukturert_json_tvinger_sentinel_verktoeyet(self):
        ...
        self.assertEqual(
            body["tool_choice"], {"type": "tool", "name": "fiq_structured_response"}
        )
        self.assertEqual(body["tools"][0]["input_schema"], schema)
    ...
    def test_tekstsvar_tolkes(self):
        response, to_call, next_inputs = ClaudeProvider.parse_message_response(
            SVAR_TEKST, []
        )
    ...
    def test_tomme_tekstblokker_forurenser_ikke_svaret(self):
        ...
        svar = {
            "content": [{"type": "text", "text": ""}, {"type": "text", "text": "Ekte."}]
        }
    ...
    def test_tomt_svar_kaster_ikke(self):
        ...
        for tomt in (None, {}, {"content": None}, {"content": []}):
            ...
            self.assertEqual(
                (response, to_call, next_inputs),
                ([], [], []),
                f"Tomt svar {tomt!r} ga ikke tomme lister",
            )
...
class TestLLMApiServicePatch(TransactionCase):
    ...
    def test_patchen_er_paa(self):
        self.assertTrue(
            getattr(LLMApiService, "_fiq_anthropic_patched", False),
    ...
    def test_base_url_er_config_drevet(self):
        ...
        self.icp.set_param(
            ClaudeProvider.BASEURL_PARAM, "https://gateway.fiq.no/anthropic/v1/"
        )
        self.addCleanup(self.icp.set_param, ClaudeProvider.BASEURL_PARAM, "")
        svc = LLMApiService(self.env, provider="anthropic")
        self.assertEqual(
            svc.base_url,
            "https://gateway.fiq.no/anthropic/v1",
    ...
    def test_anthropic_ruter_til_messages_IKKE_responses(self):
        ...
        svc = LLMApiService(self.env, provider="anthropic")
        with patch.object(
            LLMApiService, "_request", return_value=SVAR_TEKST
        ) as mock_req:
            svc._request_llm("claude-sonnet-5", ["system"], ["spørsmål"])
    ...
    def test_ugyldig_max_tokens_faller_tilbake_uten_aa_kaste(self):
        ...
        with patch.object(
            ...
            svc._request_llm("claude-sonnet-5", ["s"], ["u"])
        self.assertEqual(
            mock_req.call_args.kwargs["body"]["max_tokens"],
    ...
    def test_tool_call_response_ruter_paa_leverandoer(self):
        ...
        self.assertEqual(
            oai._build_tool_call_response("t1", "svar")["type"], "function_call_output"
        )
    ...
    def test_ugyldig_json_kropp_gir_feil_ikke_falskt_svar(self):
        ...
        for soppel in (["ikke", "et", "objekt"], "ren streng", 42):
            with patch.object(LLMApiService, "_request", return_value=soppel):
                # Bredt unntak er BEVISST (se docstringen): liste/streng/int
