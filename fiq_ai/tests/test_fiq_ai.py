...
from odoo.addons.fiq_ai.wizards.fiq_ai_setup import KEY_PARAM, CONSOLE_URL
...
class TestFiqAiShim(TransactionCase):
    ...
    def test_tomt_spoersmaal_ringer_ikke_ut(self):
        ...
        with patch.object(LLMApiService, "_request") as mock_req:
            for tomt in ("", "   ", "\n\t ", None):
                self.assertEqual(self.Ai.chat(tomt), "",
                                 "Tomt spørsmål %r ga ikke tom streng" % (tomt,))
    ...
    def test_default_grunning_er_FIQ_konteksten(self):
        ...
        with patch.object(LLMApiService, "_request", return_value=self._svar()) as mock_req:
    ...
    def test_grunningen_inneholder_de_baerende_FIQ_faktaene(self):
        ...
        for fakta in ("FIQ AI", "membran", "Vidir", "bokmål"):
            self.assertIn(fakta.lower(), FIQ_SPORAI_CONTEXT.lower(),
                          "FIQ-grunningen mangler «%s»" % fakta)
    ...
    def test_systemparameter_overstyrer_grunningen(self):
        ...
        with patch.object(LLMApiService, "_request", return_value=self._svar()) as mock_req:
            self.Ai.chat("Hei")
        self.assertEqual(mock_req.call_args.kwargs["body"]["system"], "EGEN KONTEKST FOR TEST")
    ...
    def test_tom_systemparameter_faller_tilbake_til_konstanten(self):
        ...
        with patch.object(LLMApiService, "_request", return_value=self._svar()) as mock_req:
            self.Ai.chat("Hei")
        self.assertEqual(mock_req.call_args.kwargs["body"]["system"], FIQ_SPORAI_CONTEXT)
    ...
    def test_eksplisitt_system_argument_vinner(self):
        ...
        self.assertEqual(mock_req.call_args.kwargs["body"]["system"], "Du er en salgsassistent.")
    ...
    def test_tekstsvar_returneres_som_ren_streng(self):
        with patch.object(LLMApiService, "_request", return_value=self._svar("Hei igjen.")):
            self.assertEqual(self.Ai.chat("Hei"), "Hei igjen.")

    def test_flere_tekstblokker_slaas_sammen_med_linjeskift(self):
        svar = {"content": [{"type": "text", "text": "Linje 1"},
    ...
    def test_svaret_lekker_ikke_noekkelen(self):
        ...
        with patch.object(LLMApiService, "_request", return_value=self._svar("Alt fint")):
    ...
    def test_http_feil_kastes_videre(self):
        with patch.object(LLMApiService, "_request",
                          side_effect=UserError("429 rate_limit_error")):
            with self.assertRaises(UserError) as ctx:
                self.Ai.chat("Hei")
    ...
    def test_api_feilsvar_kastes_videre(self):
        ...
        feil = {"type": "error", "error": {"type": "overloaded_error", "message": "Overloaded"}}
...
class TestFiqAiSetupWizard(TransactionCase):
    ...
    def test_tom_noekkel_avvises(self):
        ...
        self.assertIn("sk-ant", str(ctx.exception), "Feilmeldingen bør vise nøkkel-formatet")
        self.assertFalse(self.icp.get_param(KEY_PARAM),
                         "En tom nøkkel ble likevel lagret")
    ...
    def test_noekkelen_trimmes_foer_lagring(self):
        ...
        wiz = self.Wizard.create({"state": "step2",
                                  "anthropic_key": "  %s \n" % TESTNOKKEL})
        with patch.object(LLMApiService, "_request", return_value=SVAR_OK):
            wiz.action_save_and_test()
        self.assertEqual(self.icp.get_param(KEY_PARAM), TESTNOKKEL)
    ...
    def test_noekkelfeltet_toemmes_etter_lagring(self):
        ...
        with patch.object(LLMApiService, "_request", return_value=SVAR_OK):
            wiz.action_save_and_test()
        self.assertFalse(wiz.anthropic_key, "API-NØKKELEN BLE LIGGENDE I WIZARD-RECORDEN")
    ...
    def test_noekkelfeltet_toemmes_OGSAA_naar_testen_feiler(self):
        ...
        with patch.object(LLMApiService, "_request",
            ...
            wiz.action_save_and_test()
        self.assertFalse(wiz.anthropic_key,
                         "Nøkkelen ble liggende i recorden etter en feilet test")
    ...
    def test_feilet_test_KASTER_IKKE_men_forklarer(self):
        ...
        self.assertIn("Anthropic Console", wiz.test_result,
    ...
    def test_ugyldig_json_i_testen_kaster_ikke(self):
        ...
        with patch.object(LLMApiService, "_request", return_value="ikke json"):
            wiz.action_save_and_test()  # skal IKKE kaste
        self.assertTrue(wiz.test_result, "Brukeren fikk ingen tilbakemelding i det hele tatt")
    ...
    def test_testresultatet_lekker_ALDRI_noekkelen(self):
        ...
        with patch.object(LLMApiService, "_request",
            ...
            wiz.action_save_and_test()
        self.assertNotIn(TESTNOKKEL, wiz.test_result or "",
