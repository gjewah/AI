...
class FiqMgmtsystemKrav(models.Model):
    ...
    name = fields.Char(string="Krav", required=True, index=True,
                       help="Menneskelesbar tittel — navn, ikke ID.")
    klausul = fields.Char(string="Klausul", index=True,
                          help="Klausulnummer i standarden, f.eks. «7.5».")
    standard = fields.Selection([
        ...
        ("annet", "Annet / internt"),
    ], string="Standard", default="iso9001", required=True, index=True)
    beskrivelse = fields.Text(string="Beskrivelse")
    ...
    dokumentetikett = fields.Char(
        string="Dokumentetikett",
        help="Navn på documents.tag (dokumentert informasjon). Slås opp ved kjøretid.")
    kontroll_ids = fields.Many2many(
        "fiq.mgmtsystem.kontroll", "fiq_mgmtsystem_kontroll_krav_rel",
        "krav_id", "kontroll_id", string="Kontroller",
    ...
    avvik_ids = fields.One2many(
        "fiq.mgmtsystem.avvik", "krav_id", string="Avvik")
    kontroll_antall = fields.Integer(
        string="Antall kontroller", compute="_compute_antall")
    avvik_aapne = fields.Integer(
        string="Åpne avvik", compute="_compute_antall")
    ...
    def _compute_antall(self):
        for r in self:
            ...
            r.avvik_aapne = len(r.avvik_ids.filtered(
                lambda a: a.status != "lukket"))
...
class FiqMgmtsystemKontroll(models.Model):
    ...
    krav_ids = fields.Many2many(
        "fiq.mgmtsystem.krav", "fiq_mgmtsystem_kontroll_krav_rel",
        "kontroll_id", "krav_id", string="Dekker krav")
    ansvarlig_id = fields.Many2one("res.users", string="Ansvarlig")
    frekvens = fields.Selection([
        ...
        ("arlig", "Årlig"),
        ("ad_hoc", "Ved behov"),
    ], string="Frekvens", default="manedlig")
    ...
    avvik_ids = fields.One2many(
        "fiq.mgmtsystem.avvik", "kontroll_id", string="Avvik")
    company_id = fields.Many2one(
        "res.company", string="Firma", index=True, default=lambda s: s.env.company)
    aktiv = fields.Boolean(string="Aktiv", default=True, index=True)
...
class FiqMgmtsystemSjekkliste(models.Model):
    ...
    krav_id = fields.Many2one(
        "fiq.mgmtsystem.krav", string="Krav", index=True, ondelete="set null")
    er_mal = fields.Boolean(
        string="Mal", default=False,
    ...
    fremdrift = fields.Float(
        string="Fremdrift (%)", compute="_compute_fremdrift",
    ...
    def _compute_fremdrift(self):
        for r in self:
            ...
            r.fremdrift = (100.0 * r.antall_utfort / r.antall_punkt) if r.antall_punkt else 0.0
...
class FiqMgmtsystemAvvik(models.Model):
    ...
    kontroll_id = fields.Many2one(
        "fiq.mgmtsystem.kontroll", string="Kontroll", index=True, ondelete="set null")
    beskrivelse = fields.Text(string="Beskrivelse")
    ...
    alvorlighet = fields.Selection([
        ...
        ("hoy", "Høy"),
        ("kritisk", "Kritisk"),
    ], string="Alvorlighet", default="middels", index=True, tracking=True)
    status = fields.Selection([
        ("aapen", "Åpen"),
        ("under_tiltak", "Under tiltak"),
        ("lukket", "Lukket"),
    ], string="Status", default="aapen", required=True, index=True, tracking=True)
    ansvarlig_id = fields.Many2one("res.users", string="Ansvarlig", tracking=True)
    oppdaget_dato = fields.Date(
        string="Oppdaget", default=fields.Date.context_today, index=True)
    frist = fields.Date(string="Frist")
